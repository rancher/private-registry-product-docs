...
When you install the {productnameshort} {helm} chart show the following output:
[source, bash]
----
NOTES:
CHART VERSION: 1.1.6
...
[TIPs]
====
- To get the password for the admin user, you can also run the following command:
[source, bash]
kubectl get secret --namespace <PRIVATE_REGISTRY_NAMESPACE>-harbor-core <RELEASE_NAME> -o jsonpath="{.data.HARBOR_ADMIN_PASSWORD}" | base64 -d; echo
...
--namespace <PRIVATE_REGISTRY_NAMESPACE> \
--set HARBOR_ADMIN_PASSWORD=$(kubectl get secret --namespace <PRIVATE_REGISTRY_NAMESPACE>-harbor-core <RELEASE_NAME> -o jsonpath="{.data.HARBOR_ADMIN_PASSWORD}" | base64 -d)
