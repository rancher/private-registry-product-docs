...
persistence: enabled //<6>
----
[callouts]
<1> How {sregistry} is exposed. Can be `ingress`, `loadBalancer`, `nodePort` or `clusterIPhis` Default is `ingress`.
<2>  Can be `auto` or `secret`. in this case, it should be a secret like this: 
<3> To use a TLS encryption, update the field to match `externalURL`.
...
<6>  To store all the images, metadata and scans, ensure this that the `persistence`-related settings are properly configured.
