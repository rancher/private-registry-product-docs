...
Helm 4 you can use a digest to specify an chart to install
[source, bash]
  $ helm install <RELEASE_NAME> \
  oci://registry.suse.com/private-registry/private-registry-helm@sha256:<DIAGEST_OF_CHART_TO_INSTALL> \
  --namespace <PRIVATE_REGISTRY_NAMESPACE>

It’s possible to use kstatus watcher with the flag '--wait=watcher', which means that all the objects must be ready to finish the installation, so it may take a little time to do it.
...
With digest only possible with helm4
[source]
