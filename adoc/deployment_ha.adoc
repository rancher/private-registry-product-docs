...
[#pr-deployment-ha]
== Deployment with High Availibility

You can use {helm} to deploy highly available (HA) {productnameshort} on a {kube} cluster.
HA setup ensure that users do not experience interruptions of service if one of the nodes on which {productnameshort} is running becomes unavailable.

[#pr-deployment-ha-architecture]
=== Architecture of the HA setup 

Most of {productnameshort} components are now stateless.
...
For storage, users should provide an HA {postgresql} and {redis} cluster for application data, along with PVCs or object storage for storing images and charts.

image::ha-setup.png[{productnameshort} HA setup, width=75%]
...
* HA {redis} ({productnameshort} does not handle the deployment of HA {redis})
...
. Update the deployment patameters to match your requirements.
...
. Install the {produ<ctnameshort} {helm} chart.
Replace `<RELEASE_NAME>` with your custom relase name for the {helm} chart deployment.
[source]
  $ helm install <RELEASE_NAME> harbor/