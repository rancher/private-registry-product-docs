...
----
+
Replace `<your-username>` and `<your-password>` with your actual {sregistry} credentials.
...
* *Step 4:* In the air-gapped environment, load the artifacts into your {productname}:
+
[source,bash]
----
...
+
Replace:
** `<suse-private-registry-url>` with your {productname} URL (e.g., `https://registry.example.com`)
** `<registry-username>` and `<registry-password>` with your {productname} credentials
