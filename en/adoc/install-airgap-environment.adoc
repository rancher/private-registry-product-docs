...
{prompt_user}head -1 ./registry-password.txt | hauler login <SUSE_PRIVATE_REGISTRY_URL> \ <.>
  -u <REGISTRY_USERNAME> --password-stdin <.>

# Push the loaded store's content into your SUSE Private Registry project
{prompt_user}hauler store copy registry://<SUSE_PRIVATE_REGISTRY_URL>/<PROJECT_NAME>

# If your SUSE Private Registry is served over unencrypted HTTP, add --plain-http:
# hauler store copy registry://<SUSE_PRIVATE_REGISTRY_URL>/<PROJECT_NAME> --plain-http
----
